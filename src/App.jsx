...
const SAMPLE_DATA = [
  { id: "SEC-001", category: "Data Security", requirement: "All data at rest must be encrypted using AES-256 or equivalent algorithm with customer-managed keys", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "SEC-002", category: "Data Security", requirement: "Data must be encrypted in transit using TLS 1.2 or higher for all communications", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "SEC-003", category: "Data Security", requirement: "Hardware Security Module (HSM) must be used for root key protection", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "SEC-004", category: "Data Security", requirement: "Data loss prevention policies must be enforced for sensitive data", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "ACC-001", category: "Access Control", requirement: "Multi-factor authentication must be enforced for all privileged accounts accessing production", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "ACC-002", category: "Access Control", requirement: "Privileged access must follow just-in-time (JIT) access principles with time-bound approval", partnerResponse: "Not Responded", partnerComment: "" },
  ...
  { id: "ACC-004", category: "Access Control", requirement: "Role-based access control with principle of least privilege must be applied", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "AUD-001", category: "Audit & Logging", requirement: "All administrative activities must be logged and retained for a minimum of 7 years", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "AUD-002", category: "Audit & Logging", requirement: "Audit logs must be immutable and tamper-evident with cryptographic integrity", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "AUD-003", category: "Audit & Logging", requirement: "Real-time security event monitoring and alerting must be in place", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "DR-001", category: "Disaster Recovery", requirement: "Recovery Time Objective (RTO) must not exceed 4 hours for critical systems", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "DR-002", category: "Disaster Recovery", requirement: "Backups must be geo-redundant across at least two geographically separate regions", partnerResponse: "Not Responded", partnerComment: "" },
  ...
  { id: "NET-001", category: "Network Security", requirement: "Network segmentation using micro-segmentation or zero-trust principles", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "NET-002", category: "Network Security", requirement: "DDoS protection must be active for all internet-facing endpoints", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "NET-003", category: "Network Security", requirement: "Web Application Firewall must protect all public-facing web applications", partnerResponse: "Not Responded", partnerComment: "" },
  ...
  { id: "COMP-002", category: "Compliance", requirement: "Data residency must be maintained within customer-specified geographic region", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "COMP-003", category: "Compliance", requirement: "GDPR data subject rights automation (access, erasure, portability)", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "INC-001", category: "Incident Management", requirement: "Security incidents must be reported to customer within 24 hours of detection", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "INC-002", category: "Incident Management", requirement: "Dedicated 24/7 security operations center (SOC) must monitor the environment", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "VUL-001", category: "Vulnerability Management", requirement: "Critical vulnerabilities must be remediated within 7 days of discovery", partnerResponse: "Not Responded", partnerComment: "" },
  { id: "VUL-002", category: "Vulnerability Management", requirement: "Monthly vulnerability scans must be performed on all production assets", partnerResponse: "Not Responded", partnerComment: "" },
];
...
The following is an existing compliance document, policy, or standard provided by the customer. Compare our responses against this reference. Flag any contradictions, gaps, or areas where our response does not align with the reference. Note where we claim something that the reference document contradicts.
...
${referenceDoc.slice(0, 30000)}
  ...
  const prompt = `You are a senior cloud compliance architect responding to a customer's compliance questionnaire on behalf of our organisation. You are drafting responses for a ' + platform + ' deployment.

You have access to our current controls, policies, and reference documents (if provided). Your job is to:
1. Draft a professional, specific response to each customer requirement
2. Determine our compliance status honestly — Compliant, Partial, or Non-Compliant based on what we actually have in place
3. Where we ARE compliant, cite specific services, tools, and controls we use
4. Where we are NOT fully compliant, be transparent and suggest what we could implement
5. Flag items that need human review before submitting to the customer

Key principles:
- Be honest — do not claim compliance if controls are not in place
- Be specific — cite actual Azure/on-prem services, not generic statements
- Be professional — responses should be ready for customer submission after human review
- Flag gaps — clearly identify where we fall short so the team can address them.
${refSection}
For each customer requirement, assess:
1. CLARITY — can we provide a clear, specific response and specific, or vague/generic?
2. ACCURACY — are we actually compliant based on our current controls given ${platform} capabilities?${referenceDoc ? "\n2b. REFERENCE CHECK — does the response align with or contradict the reference document provided?" : ""}
...
Key principle: Azure and well-architected on-premises deployments CAN meet nearly all standard enterprise compliance requirements. Leverage Azure and on-prem capabilities where they exist (e.g. Azure Dedicated HSM, Log Analytics long-term retention, Azure Firewall for segmentation, Defender for Cloud). Be honest about gaps — the customer will verify.
  ...
  "reviews": [
    {
      ...
      "recommendedComment": "our drafted response to the customer, 1-3 sentences, cite specific services${referenceDoc ? ". Note if reference document supports or contradicts this" : ""}",
...
const exportExcel = (originalWorkbook, sheetName, requirements) => {
  ...
  if (!wb) {
    ...
    const rows = requirements.map(r => ({
      ...
      "Initial Comment": r.partnerComment,
...
const exportFormatted = (requirements, verdict, metrics, format = "pdf") => {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Compliance Review Report</title>
    ...
    <p style="color:#5A544A;font-size:14px">Generated ${new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"long",year:"numeric"})} · Sentinel Customer Compliance Response</p>
    ...
    <div>
      ...
      <div class="kpi"><div class="kpi-val">${metrics.partner.compliant}</div><div class="kpi-lab">Requirements received</div></div>
    ...
    <table>
      <tr><th>Status</th><th>Requirements</th><th>After AI Review</th><th>Change</th></tr>
    ...
    <table>
      <tr><th>ID</th><th>Category</th><th>Requirement</th><th>Initial Status</th><th>Initial Comment</th><th>AI Status</th><th>AI Comment</th><th>AI Reasoning</th></tr>
      ${rows}
    </table>

    <hr style="border:none;border-top:1px solid #E3DCC9;margin:32px 0 16px">
    <p style="font-size:11px;color:#8A8175">Sentinel · Customer Compliance Response · Powered by Claude</p>
    </body></html>`;
    ...
    <div className="max-w-[1400px] mx-auto px-8 py-5 flex items-center justify-between">
      <div className="flex items-baseline gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-display text-xl leading-none" style={{ color: T.ink, fontWeight: 500 }}>Sentinel</div>
            <div className="font-mono text-[10px] uppercase tracking-[0.2em] mt-1" style={{ color: T.inkFade }}>Customer Compliance Response</div>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-12 gap-8">
        <div className="col-span-12 md:col-span-7 fade-in">
          ...
          <h1 className="font-display text-7xl md:text-[88px] leading-[0.92] mb-6" style={{ color: T.ink, fontWeight: 400, letterSpacing: "-0.03em" }}>
            Respond to<br/>your customer's<br/>
            <span style={{ fontStyle: "italic", fontWeight: 300 }}>compliance</span> questions.
          </h1>
          <p className="text-lg leading-relaxed max-w-xl mb-10" style={{ color: T.inkSoft }}>
            Upload a customer's compliance questionnaire. AI drafts responses based on your current controls and reference documents — flagging gaps, highlighting areas that need your review, and citing specific services and policies.
          </p>
          ...
          <div
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                <div className="font-display text-xl" style={{ color: T.ink }}>Drop the customer questionnaire here</div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="text-sm font-medium" style={{ color: T.ink }}>
                  {referenceDoc ? `✓ Reference: ${refDocName}` : "Upload your control documents / policies (recommended)"}
                </div>
                <div className="text-xs mt-0.5" style={{ color: T.inkFade }}>
                  {referenceDoc
                    ? `${(referenceDoc.length / 1000).toFixed(0)}K chars loaded · AI will use this to draft accurate responses`
                    : "Upload your control documents, policies, certifications, or existing assessments as reference"}
                </div>
              </div>
              {referenceDoc && (
          ...
          <button onClick={onSample}
            ...
            style={{ color: T.ink }}>
            <span className="border-b" style={{ borderColor: T.ink }}>Or try with sample customer questionnaire</span>
        ...
        <div className="col-span-12 md:col-span-5 md:pl-8 space-y-6 fade-in" style={{ animationDelay: "0.1s" }}>
          <div className="border-l-2 pl-6" style={{ borderColor: T.ink }}>
            <div className="font-mono text-[10px] uppercase tracking-[0.25em] mb-2" style={{ color: T.inkFade }}>01 · Analyse</div>
            <div className="font-display text-2xl mb-1.5" style={{ color: T.ink, fontWeight: 500 }}>Instant verdict.</div>
            <div className="text-sm leading-relaxed" style={{ color: T.inkSoft }}>Instant assessment: can we meet this customer's requirements — fully, mostly, or with gaps — with confidence score.</div>
          </div>
          <div className="border-l-2 pl-6" style={{ borderColor: T.line }}>
            <div className="font-mono text-[10px] uppercase tracking-[0.25em] mb-2" style={{ color: T.inkFade }}>02 · Draft</div>
            <div className="font-display text-2xl mb-1.5" style={{ color: T.ink, fontWeight: 500 }}>Every line, challenged.</div>
            <div className="text-sm leading-relaxed" style={{ color: T.inkSoft }}>AI drafts a response to every question, citing your actual controls. Flags gaps where you may not comply and items that need your team's sign-off.</div>
          </div>
          <div className="border-l-2 pl-6" style={{ borderColor: T.line }}>
            <div className="font-mono text-[10px] uppercase tracking-[0.25em] mb-2" style={{ color: T.inkFade }}>03 · Submit</div>
            <div className="font-display text-2xl mb-1.5" style={{ color: T.ink, fontWeight: 500 }}>Same file, upgraded.</div>
            <div className="text-sm leading-relaxed" style={{ color: T.inkSoft }}>Export the completed questionnaire with your drafted responses, ready for internal review and customer submission.</div>
          </div>
        </div>
      </div>
    </div>
...
const AnalysingView = ({ step, total, platform }) => (
  <div className="max-w-[900px] mx-auto px-8 py-24 text-center fade-in">
    ...
    <div className="font-mono text-xs uppercase tracking-[0.25em] mb-5" style={{ color: T.inkFade }}>
      {platform} · AI drafting responses
    </div>
    <h2 className="font-display text-5xl mb-4" style={{ color: T.ink, fontWeight: 400 }}>
      Drafting your responses<span style={{ fontStyle: "italic" }}>…</span>
    </h2>
    <p className="text-lg max-w-xl mx-auto mb-10" style={{ color: T.inkSoft }}>
      Analysing {total} customer requirements against your controls, reference documents, and industry best practices to draft compliant responses.
    </p>
...
/* ───────────────────────── UI: Verdict banner ───────────────────────── */
const VERDICT_META = {
  can_be_compliant: { label: "Fully addressable", tone: T.emerald, toneBg: T.emeraldBg, glyph: "✓" },
  mostly_compliant: { label: "Mostly covered",    tone: T.amber,   toneBg: T.amberBg,   glyph: "◐" },
  challenging:      { label: "Gaps identified",     tone: T.rust,    toneBg: T.rustBg,    glyph: "△" },
};
...
const ComparisonChart = ({ metrics }) => {
  const data = [
    { name: "Compliant",     Requirements: metrics.partner.compliant,     "After AI": metrics.ai.compliant },
    { name: "Partial",       Requirements: metrics.partner.partial,       "After AI": metrics.ai.partial },
    { name: "Non-Compliant", Requirements: metrics.partner.nonCompliant,  "After AI": metrics.ai.nonCompliant },
    { name: "Not Responded", Requirements: metrics.partner.notResponded,  "After AI": metrics.ai.notResponded },
  ];
  return (
    <div className="p-6" style={{ background: T.card, border: `1px solid ${T.line}` }}>
      <div className="flex items-baseline justify-between mb-4">
        ...
        <div className="flex gap-4 text-xs">
          <div className="flex items-center gap-2"><div className="w-3 h-3" style={{ background: T.inkFade }} /><span style={{ color: T.inkSoft }}>Status</span></div>
      ...
      <ResponsiveContainer width="100%" height={240}>
        <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <Bar dataKey="Requirements" fill={T.inkFade} />
  ...
  return (
    <div className="p-6 relative" style={{ background: T.card, border: `1px solid ${T.line}` }}>
      ...
      <div className="flex items-center justify-around gap-2">
        <MiniDonut data={partnerData} pct={partnerPct} label="Before" />
  ...
  return (
    <div className="fade-in">
      <div className="flex items-baseline justify-between mb-5 flex-wrap gap-4">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.25em] mb-1" style={{ color: T.inkFade }}>Response draft</div>
          <h3 className="font-display text-3xl" style={{ color: T.ink, fontWeight: 500 }}>Every question, answered</h3>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select value={filter} onChange={e => setFilter(e.target.value)}
            ...
            <option value="improved">AI addressed</option>
            <option value="unclear">Unclear</option>
            <option value="attention">Needs your review</option>
          ...
          <button onClick={() => setShowPartnerDetail(!showPartnerDetail)}
            ...
            <Search size={14} /> {showPartnerDetail ? "Hide drafts" : "Show drafts"}
          </button>
        </div>
      </div>
            ...
            {showPartnerDetail && (
              <div className="px-5 pb-4 grid grid-cols-2 gap-3" style={{ background: T.lineSoft + "60" }}>
                <div className="p-3" style={{ background: T.card, border: `1px solid ${T.line}` }}>
                  <div className="font-mono text-[9px] uppercase tracking-wider mb-1" style={{ color: T.inkFade }}>Customer asked</div>
                ...
                <div className="p-3" style={{ background: "#FBF7EC", border: `1px solid ${T.line}` }}>
                  <div className="font-mono text-[9px] uppercase tracking-wider mb-1 flex items-center gap-1" style={{ color: T.inkFade }}><Sparkles size={8} /> Our drafted response</div>
        ...
        <div className="p-8">
          ...
          <div className="p-5 mb-5" style={{ background: T.card, border: `1px solid ${T.line}` }}>
            <div className="flex items-center justify-between mb-3">
              <div className="font-mono text-[10px] uppercase tracking-[0.2em]" style={{ color: T.inkFade }}>Customer asked</div>
          ...
          {req.aiRecommendedStatus && (
            <div className="p-5 mb-5 relative" style={{ background: "#FBF7EC", border: `1px solid ${T.ink}` }}>
              <div className="absolute -top-3 left-5 px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider flex items-center gap-1" style={{ background: T.ink, color: T.bg }}>
                <Sparkles size={10} /> AI Response Draft
              </div>
              ...
              <div className="mb-4">
                <div className="font-mono text-[10px] uppercase tracking-[0.2em] mb-2" style={{ color: T.inkFade }}>Drafted response</div>
          ...
          <div className="flex gap-2">
            {req.aiRecommendedStatus && (
              <button onClick={acceptAI} className="flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium"
                style={{ background: T.ink, color: T.bg }}>
                <Check size={14} /> Accept drafted response
              </button>
            )}
...
Return ONLY a JSON object with this exact structure:
{
  ...
  "complianceScoreAnalysis": {
    "beforeSummary": "1-2 sentences on our initial compliance position",
...
const buildExecPDF = (report, metrics, verdict, platform) => {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${report.title}</title>
    ...
    <div class="cover">
      ...
      <div class="meta">${d} · Sentinel Customer Compliance Response</div>
    ...
    <div class="footer">
      <p>Sentinel · Customer Compliance Response · Powered by Claude · ${d}</p>
...
const buildExecPPTX = (report, metrics, verdict, platform) => {
  ...
  const slide = (content, bg = "#14120F", color = "#F7F3EA") => `
    <div style="width:960px;height:540px;background:${bg};color:${color};padding:48px 60px;display:flex;flex-direction:column;justify-content:center;page-break-after:always;position:relative;box-sizing:border-box">
      ${content}
      <div style="position:absolute;bottom:20px;right:60px;font-size:10px;opacity:.5;font-family:monospace">Sentinel · ${d}</div>
    </div>`;
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${report.title} - Slides</title>
    ...
    ${slide(`
      ...
      <div style="margin-top:40px;font-size:12px;opacity:.4">${d} · Sentinel Customer Compliance Response</div>
    `)}
    ...
    ${slide(`
      <div style="text-align:center">
        ...
        <div style="margin-top:40px;font-size:12px;opacity:.3">Sentinel · Customer Compliance Response · Powered by Claude</div>
      </div>
    `)}
  </body></html>`;
      ...
      <div className="w-full max-w-4xl my-8 mx-4" style={{ background: T.bg }} onClick={e => e.stopPropagation()}>
        ...
        <div className="p-8 border-b" style={{ borderColor: T.line, background: T.ink, color: T.bg }}>
          ...
          <div className="flex gap-6 mt-6">
            <div className="text-center"><div className="font-display text-4xl opacity-50">{partnerPct}%</div><div className="font-mono text-[9px] uppercase tracking-wider opacity-30 mt-1">Status</div></div>
            <div className="flex items-center opacity-30">→</div>
            <div className="text-center"><div className="font-display text-4xl" style={{ color: "#7dcd6e" }}>{aiPct}%</div><div className="font-mono text-[9px] uppercase tracking-wider opacity-30 mt-1">Our Response</div></div>
        ...
        <div className="p-6 border-t flex items-center justify-between" style={{ borderColor: T.line }}>
          <span className="font-mono text-[10px]" style={{ color: T.inkFade }}>Sentinel · Customer Compliance Response</span>
          <div className="flex gap-2">
...
export default function App() {
  ...
  const runReview = async (reqs, wb, sn) => {
    ...
    try {
      ...
      setStep("Matching requirements to your controls…");
      ...
      {view === "dashboard" && verdict && (
        <main className="max-w-[1400px] mx-auto px-8 py-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="p-5" style={{ background: T.card, border: `1px solid ${T.line}` }}>
              <div className="font-mono text-[10px] uppercase tracking-[0.2em] mb-3 flex items-center gap-2" style={{ color: T.inkFade }}>
                <span className="w-2 h-2 rounded-full" style={{ background: T.inkFade }} /> Customer Requirements Breakdown
              </div>
            ...
            <div className="p-5" style={{ background: "#FBF7EC", border: `1px solid ${T.ink}` }}>
              <div className="font-mono text-[10px] uppercase tracking-[0.2em] mb-3 flex items-center gap-2" style={{ color: T.ink }}>
                <Sparkles size={10} /> Our Drafted Response Status
              </div>
              <div className="grid grid-cols-4 gap-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
            ...
            <MetricCard label="Total questions" value={metrics.responded} sub={`${metrics.notResponded} not responded`} />
            <MetricCard label="AI addressed" value={metrics.improved} accent={T.emerald} sub="Responses drafted by AI" />
            <MetricCard label="Needs your review" value={metrics.needsAttention} accent={T.rust} sub="Gaps or items requiring sign-off" />
          </div>
          ...
          <div className="mt-10 pt-6 border-t flex items-center justify-between text-xs font-mono" style={{ borderColor: T.line, color: T.inkFade }}>
            <span>Sentinel · Customer Compliance Response · {new Date().toLocaleDateString("en-GB", { day:"2-digit", month:"short", year:"numeric" })}</span>
            <span>Powered by Claude · Customer Compliance Response Agent</span>
          </div>
        </main>
      )}
